...
from zsmall_msgs.action import zsmallTask
...
class TaskServer(Node):
    def __init__(self):
        ...
        self.action_server = ActionServer(
            self, zsmallTask, "task_server", self.goalCallback
        )

        # MoveIt 2 Interface
        self.zsmall = MoveItPy(node_name="moveit_py")
        self.zsmall_arm = self.zsmall.get_planning_component("arm")
        self.zsmall_gripper = self.zsmall.get_planning_component("gripper")
    ...
    def goalCallback(self, goal_handle):
        ...
        arm_state = RobotState(self.zsmall.get_robot_model())
        gripper_state = RobotState(self.zsmall.get_robot_model())
        ...
        self.zsmall_arm.set_start_state_to_current_state()
        self.zsmall_gripper.set_start_state_to_current_state()

        self.zsmall_arm.set_goal_state(robot_state=arm_state)
        self.zsmall_gripper.set_goal_state(robot_state=gripper_state)

        arm_plan_result = self.zsmall_arm.plan()
        gripper_plan_result = self.zsmall_gripper.plan()

        if arm_plan_result and gripper_plan_result:
            self.get_logger().info("Planner SUCCEED, moving the arm and the gripper")
            self.zsmall.execute(arm_plan_result.trajectory, controllers=[])
            self.zsmall.execute(gripper_plan_result.trajectory, controllers=[])
        else:
            self.get_logger().info("One or more planners failed!")
        
        self.get_logger().info("Goal succeeded")
        goal_handle.succeed()
        result = zsmallTask.Result()
        result.success = True
        return result 
